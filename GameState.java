...
public class GameState implements Cloneable{
   ...
   if (players.size() > 4 || players.size() < 2)     {
    ...
    int Kingdom_Cards_Selected=3;

    // We only defined Adventurer, smithy, and Village. We need to define
    //more kingdom cards the Card class we should change 3 to the  exact of the
    // number of kingdomcards. look at the requirements of the assignment-1
  ...
  public boolean isGameOver() {
    ...
    for (int i : gameBoard.values()){
      if (i == 0)
        emptySupplyPile++;
        if ( emptySupplyPile >= 3){ return true;}
      }
      return false;
    }
    ...
    @Override public String toString() {
        ...
        if (gameBoard.isEmpty())
          ...
          for (Player player : players) sb.append(" --- " + player.toString() + "\n");
            sb.append(" --- gameBoard --- \n");
